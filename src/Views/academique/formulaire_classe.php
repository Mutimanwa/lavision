            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="card shadow-sm">
                        ...
                        <div class="card-body">
                            <form method="post" action="<?php echo BASE_URL; ?>/academique/classes/<?php echo $data['mode'] === 'ajout' ? 'ajouter' : 'modifier/' . ($data['valeurs']['id'] ?? ''); ?>" id="formClasse">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-select <?php echo isset($data['erreurs']['id_section']) ? 'is-invalid' : ''; ?>"
                                                id="id_section" name="id_section" required>
                                            <option value="">Sélectionner une section</option>
                                            <?php foreach ($data['sections'] as $section): ?>
                                                <option value="<?php echo $section['id']; ?>"
                                                        <?php echo (isset($data['valeurs']['id_section']) && $data['valeurs']['id_section'] == $section['id']) ? 'selected' : ''; ?>>
                                                    <?php echo htmlspecialchars($section['nom_section']); ?> (<?php echo htmlspecialchars($section['code_section']); ?>)
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                        <?php if (isset($data['erreurs']['id_section'])): ?>
                                            <div class="invalid-feedback">
                                                <?php echo htmlspecialchars($data['erreurs']['id_section']); ?>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                    ...
                                    <!-- Année scolaire -->
                                    <div class="col-md-6 mb-3">
                                        <label for="id_annee_scolaire" class="form-label">
                                            Année scolaire <span class="text-danger">*</span>
                                        </label>
                                        <select class="form-select <?php echo isset($data['erreurs']['id_annee_scolaire']) ? 'is-invalid' : ''; ?>"
                                                id="id_annee_scolaire" name="id_annee_scolaire" required>
                                            <option value="">Sélectionner une année</option>
                                            <?php foreach ($data['annees_scolaires'] as $annee): ?>
                                                <option value="<?php echo $annee['id']; ?>"
                                                        <?php echo (isset($data['valeurs']['id_annee_scolaire']) && $data['valeurs']['id_annee_scolaire'] == $annee['id']) ? 'selected' : ''; ?>>
                                                    <?php echo $annee['annee_debut']; ?> - <?php echo $annee['annee_fin']; ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                        <?php if (isset($data['erreurs']['id_annee_scolaire'])): ?>
                                            <div class="invalid-feedback">
                                                <?php echo htmlspecialchars($data['erreurs']['id_annee_scolaire']); ?>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    <a href="<?php echo BASE_URL; ?>/academique/classes" class="btn btn-outline-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    ['id_niveau', 'id_section', 'id_annee_scolaire'].forEach(function(fieldId) {
    ...
    form.addEventListener('submit', function(e) {
        ...
        const requiredFields = ['nom_classe', 'id_niveau', 'id_section', 'id_annee_scolaire'];
